...
const tdom = (function (factory) {
  ...
  const self = {
    ...
    connectLoadObserver($content) {
      ...
      loadObserver = new MutationObserver(((mutations) => {
        newMutations = true;
        ...
        for (const m of mutations) {
          if (m.addedNodes.length === 1 && m.target.className === 'js-plugin-badges'
            ...
            const theList = $(self.getContainingList(m.target));
    ...
    getLists(name, filter) {
      ...
      if (filter !== undefined) {
        jLists = jLists.filter(function () {
          const titleText = $(this).find('h2').text();
    ...
    getCardName($card) {
      ...
      const $span = $card.find('span.list-card-title');
      if ($span.length === 0) {
        return;
      }
      const title = $card.find('span.list-card-title')
    ...
    getCardsByName(name, exactMatch = false) {
      ...
      const jCards = $('a.list-card').filter(function () {
        const title = self.getCardName($(this));
        if (exactMatch) {
          return title === name;
        }
